...
def get_system_prompt():
    sys_prompt = """你是一个乐于助人的助手。你首先在脑海中思考推理过程，然后为用户提供答案。推理过程和答案分别用<think> </think>和<answer> </answer>标签括起来。标签之外不要有其他内容。"""
    return sys_prompt

def get_task_prompt():
    task_prompt_1 = """请从网络安全的角度分析下面的网络流量，回答结果必须为(攻击失败|攻击成功|非攻击)三者之一。\n"""
    task_prompt_2 = """请从网络安全的角度分析下面的网络流量, 在<think>...</think>部分中，首先分析 http 请求的功能，然后分析 http 的响应，首先判断此次http流量是不是正常业务，然后考虑是否构成一次攻击事件。假如是正常业务，答案为<answer>非攻击</answer>。假如是攻击事件，进一步判断是否攻击成功。最后给出的答案为(非攻击|攻击成功|攻击失败)三者之一，答案部分不要有任何其他内容。\n"""
    
    task_prompts = [task_prompt_1, task_prompt_2]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='data/ndr/json/ndr-simon-sxz-r1-human.jsonl')
    parser.add_argument('--train_size', type=int, default=2000)
    parser.add_argument('--test_size', type=int, default=270)
    ...
    # 随机化数据集
    raw_dataset = raw_dataset.shuffle(seed=47)  # 指定种子值以确保可重复性
